...
#all_projects_df = pd.read_csv("all_projects_top_30_General_4_8.csv")

#id_project_list = list(set(all_projects_df['id_project'].to_list()))

id_project_list = [15335, 991]
print(len(id_project_list))

all_projects_df = pd.DataFrame(columns=['id_project', 'name'])
all_projects_df['id_project'] = id_project_list
all_projects_df['name'] = ['Moonbeam', 'Ethereum Classic']
...
pd.DataFrame(data).to_csv('top_30_performances_2_4_2projects_General.csv', sep=',', index=False)